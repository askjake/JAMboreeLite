"""Atomic, additive persistence for JAMboree ``base.txt``.

The file mixes stable receiver identity, mutable network information, DART wiring,
and optional legacy SGS credentials.  Writes are serialized across threads and
processes, written through a temporary file, and backed up before replacement.
"""
...
class BaseFileError(RuntimeError):
    """Base class for configuration persistence errors."""


class BaseFileCorruptError(BaseFileError):
    """Raised when neither the primary nor backup file contains valid JSON."""
...
def _parse_document(text: str, source: Path) -> Dict[str, Any]:
    ...
    if "stbs" in value and not isinstance(value["stbs"], dict):
...
def read_document(path: Path) -> Dict[str, Any]:
    """Read the primary document, falling back to ``.bak`` when corrupt."""
    path = Path(path)
    ...
    if backup.is_file():
        try:
            ...
            raise BaseFileCorruptError(f"both {path} and {backup} are unreadable") from exc
...
def _atomic_write(path: Path, payload: str) -> None:
    ...
    try:
        ...
        # Persist the directory entry on POSIX when possible.
        if os.name != "nt":
            try:
...
def write_document(path: Path, document: Mapping[str, Any]) -> None:
    ...
    _parse_document(payload, path)
    if path.is_file():
        try:
            current = path.read_text(encoding="utf-8")
            _parse_document(current, path)
        except (OSError, BaseFileCorruptError):
            current = ""
        if current:
            _atomic_write(
                Path(f"{path}.bak"), current if current.endswith("\n") else current + "\n"
            )
    ...
    *,
    ...
    with _thread_lock, _FileLock(path):
        ...
        if not isinstance(stbs[alias], dict):
            raise BaseFileCorruptError(f"STB entry {alias!r} is not an object")
...
def replace_stb_table(
    ...
    *,
    ...
    """Replace the visible STB table while retaining protected hidden fields."""
    if not isinstance(stbs, Mapping):
        raise TypeError("stbs must be a mapping")
    ...
    with _thread_lock, _FileLock(path):
        ...
        for raw_alias, raw_incoming in stbs.items():
            ...
            if not isinstance(raw_incoming, Mapping):
                raise TypeError(f"STB entry {alias!r} must be a mapping")
            incoming = dict(raw_incoming)
            prior = dict(previous.get(alias, {}) or {})
            entry = dict(prior)
...
def get_plaintext_credentials(path: Path, alias: str) -> Optional[tuple[str, str]]:
